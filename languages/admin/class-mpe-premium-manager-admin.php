...
namespace XackiGiFF\PremiumManager\admin;
...
class MPEPremiumManagerAdmin
{
    ...
    public function add_admin_menu(): void
    {
        ...
        add_submenu_page(
            ...
            array($this, 'render_admin_manage_settings_page'));
    }
    ...
    public function render_admin_manage_settings_page(): void
    {
        require_once MPE_PREMIUM_MANAGER_DIR . 'admin/view/mpe-premium-manager-admin-settings-display.php';
    }
    ...
    public static function sort_users($sort_order): array
    {
        return match ($sort_order) {
            'active' => array('orderby' => 'premium_status', 'order' => 'DESC'),
            'inactive' => array('orderby' => 'premium_status', 'order' => 'ASC'),
            'users_newest' => array('orderby' => 'user_registered', 'order' => 'DESC'),
            'users_oldest' => array('orderby' => 'user_registered', 'order' => 'ASC'),
            default => array('orderby' => 'ID', 'order' => 'DESC'),
        };
    }

    private static function display_sort_buttons($current_page): void
    {
    ...
    public static function display_premium_users_table($current_page = 1, $per_page = 20, $sort_order = 'default'): void
    {
        ...
        $table_html .= '<th>Manage</th>';  // Новая колонка для управления
        $table_html .= '</tr>';
        $table_html .= '</thead>';
        $table_html .= '<tbody>';
        ...
        foreach ($users as $user) {
            ...
            $table_html .= '<td><button class="manage-edit" data-user-id="' . esc_attr($user_id) . '">Edit</button></td>';  // Кнопка "Edit"
            $table_html .= '</tr>';
        }
